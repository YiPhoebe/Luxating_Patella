import os, sys, yaml, torch, time, contextlib
...
def build_transforms(input_size, train=True):
    if train:
        return A.Compose([
            A.ShiftScaleRotate(shift_limit=0.02, scale_limit=0.05, rotate_limit=7, border_mode=0, p=0.7),
...
def main(cfg_path="./config/config.yaml"):
    ...
    train_ds = KneeXrayDataset(cfg["data"]["train_jsons"], cfg["data"]["image_root"],
                               input_size=cfg["data"]["input_size"], use_bbox=cfg["data"]["use_bbox"],
                               train=True, transforms=build_transforms(cfg["data"]["input_size"], True))
    val_ds = KneeXrayDataset(cfg["data"]["val_jsons"], cfg["data"]["image_root"],
                             input_size=cfg["data"]["input_size"], use_bbox=cfg["data"]["use_bbox"],
                             train=False, transforms=build_transforms(cfg["data"]["input_size"], False))
    ...
    model = build_model(cfg["model"]["name"], cfg["model"]["pretrained"], cfg["model"]["num_classes"]).to(device)
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        model.train()
        t0 = time.time(); n_samples = 0
        for x, y in tqdm(train_loader, desc=f"Epoch {epoch} [train]"):
            x, y = x.to(device), y.to(device)
            ctx = torch.autocast("mps", dtype=torch.float16) if use_amp else contextlib.nullcontext()
            with ctx:
                logits = model(x)
        ...
        # 검증 단계
        model.eval(); accs = []; precs = []; recs = []; f1s = []
        all_logits = []
        all_labels = []
        with torch.no_grad():
        ...
        if f1 > best_f1:
            ...
            torch.save({"model": model.state_dict(), "config": cfg}, os.path.join(cfg["train"]["save_dir"], "resnet18_best.pt"))
            # 검증 세트 기준 Temperature Scaling 적용 및 저장
            try:
                logits_cat = torch.cat(all_logits, dim=0)
                labels_cat = torch.cat(all_labels, dim=0)
                T = fit_temperature_from_logits(logits_cat, labels_cat)
                save_calibration(T, os.path.join(cfg["train"]["save_dir"], "calib.json"))
