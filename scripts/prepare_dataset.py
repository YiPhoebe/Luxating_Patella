...
def _find_label_json(labels_index: Optional[Dict[str, Path]], stem: str) -> Optional[Path]:
...
def _extract_bbox_and_label(obj: Dict[str, Any]) -> Tuple[Optional[List[float]], Optional[int]]:
            ...
            for lk in ("label", "class", "category_id"):
                if lk in obj:
                    try:
                        ...
                        label = 1 if str(obj[lk]).lower() in ("abn", "abnormal", "1", "true") else 0
                    break
            return bbox, label
    # 1) 커스텀/COCO 유사: annotations 안에서 bbox 또는 points 처리
    if "annotations" in obj and isinstance(obj["annotations"], list) and obj["annotations"]:
        # 우선 bbox 필드가 있으면 사용
        ann0 = obj["annotations"][0]
        if isinstance(ann0, dict):
            if "bbox" in ann0 and isinstance(ann0["bbox"], (list, tuple)) and len(ann0["bbox"]) == 4:
                bbox = [float(v) for v in ann0["bbox"]]
                return bbox, label_from_meta
            # points 두 점(좌상/우하) → [x,y,w,h] 변환
            if "points" in ann0 and isinstance(ann0["points"], list) and len(ann0["points"]) >= 2:
    ...
    if "shapes" in obj and isinstance(obj["shapes"], list) and obj["shapes"]:
        ...
        if isinstance(shp, dict):
            ...
            if "points" in shp and isinstance(shp["points"], list) and len(shp["points"]) >= 2:
                (x1, y1), (x2, y2) = shp["points"][0], shp["points"][1]
                bbox = [float(min(x1, x2)), float(min(y1, y2)), float(abs(x2 - x1)), float(abs(y2 - y1))]
                label = None
                if "label" in shp:
                    try:
                        label = int(shp["label"])  # 숫자 가능 시
                    except Exception:
                        label = 1 if str(shp["label"]).lower() in ("abn", "abnormal", "1", "true") else 0
                return bbox, label
    return None, label_from_meta


def to_records(files: List[Path], image_root: Path, labels_index: Optional[Dict[str, Path]] = None, log_prefix: str = "") -> List[dict]:
    recs = []
    for i, f in enumerate(files, 1):
        try:
            label = infer_label_from_name(f)
            rel = f.resolve().relative_to(image_root.resolve())
            rec: Dict[str, Any] = {"image_path": str(rel).replace(os.sep, "/"), "label": int(label)}
...
def main():
    ap = argparse.ArgumentParser(description="무릎 X-ray 데이터셋을 위한 학습/검증 JSON 생성 스크립트")
    ap.add_argument("--image-root", type=str, required=True, help="이미지 루트 디렉토리")
    ...
    ap.add_argument("--train-dir", type=str, default=None, help="(선택) 학습 이미지가 있는 디렉토리")
    ap.add_argument("--val-dir", type=str, default=None, help="(선택) 검증 이미지가 있는 디렉토리")
    ap.add_argument("--labels-dir", type=str, default=None, help="(선택) 이미지와 매칭되는 라벨 JSON들이 있는 디렉토리(dog_id별 JSON)")
    ap.add_argument("--max-train", type=int, default=None, help="(선택) 학습용 최대 처리 이미지 수(디버그용)")
    ap.add_argument("--max-val", type=int, default=None, help="(선택) 검증용 최대 처리 이미지 수(디버그용)")
    ap.add_argument("--split", type=float, default=0.9, help="단일 루트 사용 시 학습 데이터 비율")
    args = ap.parse_args()
    ...
    if args.train_dir or args.val_dir:
        if not (args.train_dir and args.val_dir):
            raise SystemExit("명시적 디렉토리를 사용할 경우 --train-dir 와 --val-dir 를 모두 지정해야 합니다")
        ...
        print(f"학습 이미지: {len(train_files)}개, 검증 이미지: {len(val_files)}개", flush=True)
        train_recs = to_records(train_files, image_root, labels_index, log_prefix="[train]")
        ...
        if args.max_train or args.max_val:
            print("경고: 단일 루트 분할 모드에서 --max-train/--max-val은 무시됩니다", flush=True)
